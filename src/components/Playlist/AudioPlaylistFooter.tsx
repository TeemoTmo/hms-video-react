...
import { AudioPlaylistIcon, CloseIcon, PlaylistIcon, UploadIcon } from '../Icons';
...
export interface AudioPlaylistFooterClasses {
  root?: string;
  header?: string;
  body?: string;
  footer?: string;
...
export interface AudioPlaylistFooterProps {
  classes: AudioPlaylistFooterClasses;
  selectedAudios: (arg: any) => void;
}
...
export const AudioPlaylistFooter = ({ classes, selectedAudios }: AudioPlaylistFooterProps) => {
  const { tw } = useHMSTheme();
  const styler = useMemo(
    () =>
      hmsUiClassParserGenerator<AudioPlaylistFooterClasses>({
        tw,
        classes,
        defaultClasses,
  ...
  const changeHandler = (event: any) => {
    ...
    selectedAudios(event.target.files ? event.target.files : []);
      ...
      >
        <div className={styler('root')}>
          <div className={styler('header')}>
            ...
            <input name="" type="file" accept="audio/*" ref={fileInputRef} onChange={changeHandler} id="formId" hidden />
            <Button
              key="audioPlaylist"
              iconOnly
              variant="no-fill"
          ...
          <div
            className={`${styler('body')} ${collapse ? styler('collapse') : ''
              }`}
          >
            {playlist.map(item => {
              return (
